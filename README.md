...
```bash
cd /root/haidm/pest-pvt-haidm

# 1. Tải pretrained (~89 MB)
mkdir -p pretrained
wget https://github.com/whai362/PVT/releases/download/v2/pvt_v2_b2.pth -O pretrained/pvt_v2_b2.pth

# 2. Cài env (Python 3.8/3.9, CUDA phù hợp với server)
pip install torch==1.10.0+cu111 torchvision==0.11.0+cu111 -f https://download.pytorch.org/whl/torch_stable.html
pip install mmcv-full==1.3.18 -f https://download.openmmlab.com/mmcv/dist/cu111/torch1.10.0/index.html
pip install mmdet==2.13.0 timm einops

# 3. Train (1 GPU)
bash dist_train.sh configs/atss_pvtv2_dyhead3_ass.py 1

# 4. Eval
bash dist_test.sh configs/atss_pvtv2_dyhead3_ass.py work_dirs/atss_pvtv2_dyhead3_ass/latest.pth 1 --eval mAP
```

### Lưu ý

- **Backbone**: `train1.py` đang import `pvt_v2` (không SSA), `test1.py` import `pvt_v2_ssa`. Cả hai đều đăng ký `pvt_v2_b2` cùng tên. Paper nhấn mạnh đóng góp SSA — nếu muốn train đúng paper, bạn nên đổi `train1.py` dòng `import pvt_v2` → `import pvt_v2_ssa`. Tôi giữ nguyên repo gốc, bạn cân nhắc.
- **lr=1e-4** trong config là cho 8 GPU. Nếu chạy 1 GPU, cân nhắc giảm xuống `lr=1.25e-5` (linear scaling) hoặc giữ nguyên nếu batch size mỗi GPU vẫn lớn.
- `samples_per_gpu=8` trong `pest24.py` đã giảm từ 40 (gốc 8×GPU) cho phù hợp 1 GPU. Tăng/giảm theo VRAM server.
